...
export function logAuthError(error: unknown, context: string, additionalData?: Record<string, unknown>): void {
  ...
  console.group(`🔐 Auth Error [${errorResult.severity.toUpperCase()}] - ${context}`);
  console.error('Original Error:', error);
  console.log('User Message:', errorResult.message);
  console.log('Suggested Action:', errorResult.action);
  console.log('Error Code:', errorResult.originalCode);
  if (additionalData) {
